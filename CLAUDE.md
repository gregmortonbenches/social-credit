...
| 18 | Prosperity visualiser | `WheatField.tsx` renders three tiled copies of a Noun Project crop SVG (noun-crops-7578613) side by side using `SvgXml` from `react-native-svg`. Height 180px. Grown crop `#C89600` over fallow ground `#D5C9AC`, revealed left-to-right in proportion to `quotaPercent`, with a hairline marker at the fill edge. Ground lines intentionally overlap between copies. Attribution text excluded from rendered XML. **Opacity was the original encoding** (0.55 at 0% to 1.0 at 100%) — people read opacity poorly, so 0% and 40% were near-indistinguishable and the panel's main number was carried by a meter nobody could read. The overlay clips rather than scales; scaling would squash the crop instead of revealing more of it. |
