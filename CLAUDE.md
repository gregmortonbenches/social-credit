...
**Collective timezone is the source of truth for all scheduling.** Draft windows, weekly resets, and task due dates all run on the collective's configured IANA timezone (set at creation from the creator's device). Device timezone is only used for notification quiet hours.
